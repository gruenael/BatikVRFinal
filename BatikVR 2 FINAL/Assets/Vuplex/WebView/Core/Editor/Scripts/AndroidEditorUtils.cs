...
namespace Vuplex.WebView.Editor {
    ...
    public static class AndroidEditorUtils {
        ...
        static void _assertThatSrpBatcherIsDisabled() {
                ...
                // Checking defaultRenderPipeline is needed to verify that URP is enabled because useScriptableRenderPipelineBatching
                // can sometimes be true even when the built-in render pipeline is in use.
                if (GraphicsSettings.useScriptableRenderPipelineBatching && GraphicsSettings.defaultRenderPipeline != null) {
